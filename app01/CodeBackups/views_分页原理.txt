...
def prettynum_list(request):
    ...
    #     models.PrettyNum.objects.create(phonenum=str(13457882431 + num), price=10 + num, level=1, status=0)
    ...
    total_page_count, remainder = divmod(total_count, 10)
    if remainder:
        total_page_count += 1
    page = request.GET.get("page", "1")
    # 跳转页码数据验证
    page_jump_error = ""
    if (not page.isdigit()) or (page.isdigit() and (int(page) <= 0 or int(page) > total_page_count)):  # str.isdigit()判断字符串是否只有数字
        page_jump_error = "整数页码范围[1-" + str(total_page_count) + "]"
        page = 1

    page = int(page)  # 当前页
    start = (page - 1) * 10  # 区间最小值
    end = page * 10  # 区间最大值[不含]

    # 指定显示行数
    querySet = models.PrettyNum.objects.filter(**data_dict).order_by("id")[start:end]  # fillter不传入值，同样获取全部。
    # 显示当前页前5页与后5页
    plus = 5
    # 数据<=11页，显示到最大页
    ...
    # 生成页码按钮,附带首页
    page_str_lst = ['<li><a href="?page={}">首页</a></li>'.format(1)]
    # 上一页
    if page > 1:
    ...
    page_str_lst.append(prev)
    # 尾页
    page_str_lst.append('<li><a href="?page={}">尾页</a></li>'.format(total_page_count))

    # 字符转html代码处理
    page_str = mark_safe("".join(page_str_lst))

    return render(request, "pretty_list.html", {"querySet": querySet, "search_data": search_data, "page_str": page_str, "page_jump_error": page_jump_error, "page": page})
